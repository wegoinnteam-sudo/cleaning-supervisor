...
const linenItems = [
  { key: 'singleDuvetCover', labels: ['싱글 이불커버', '싱글이불커버', 'single duvet cover'], packSize: 5 },
  { key: 'doubleDuvetCover', labels: ['더블 이불커버', '더블이불커버', 'double duvet cover'], packSize: 5 },
  { key: 'singleMattressCover', labels: ['싱글 매트리스커버', '싱글매트리스커버', '싱글 배트리스커버', 'single mattress cover'], packSize: null },
  { key: 'doubleMattressCover', labels: ['더블 매트리스커버', '더블매트리스커버', 'double mattress cover'], packSize: null },
  { key: 'pillowCover', labels: ['베개커버', 'pillow cover'], packSize: 50 },
  { key: 'bathMat', labels: ['발매트', 'bath mat'], packSize: 20 },
]
...
async function saveLinenInventory(env, { requiredQuantities = {}, receivedInputs = {} }) {
  ...
  if (!data.length) throw new Error('저장할 세탁필요수량, 들어온수량 또는 현재재고 컬럼을 찾지 못했습니다.')
